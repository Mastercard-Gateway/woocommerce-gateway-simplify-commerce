...
class WC_Addons_Gateway_Simplify_Commerce extends WC_Gateway_Simplify_Commerce {
	...
	protected function process_subscription( $order, $cart_token = '' ) {
		...
		try {
			...
			if ( is_object( $customer ) && '' != $customer->id ) {
				$this->save_subscription_meta( $order_id, $customer->id );
			} else {
	...
	protected function process_pre_order( $order, $cart_token = '' ) {
		...
		if ( WC_Pre_Orders_Order::order_requires_payment_tokenization( $order_id ) ) {
			...
			try {
				if ( absint( $order->get_total() * 100 ) < 50 ) {
	...
	public function process_payment( $order_id ) {
		...
		} elseif ( 'standard' == $this->mode && $this->order_contains_pre_order( $order_id ) ) {
	...
	public function process_subscription_payment( $order, $amount = 0 ) {
		...
		$customer_id = get_post_meta( $order_id, '_simplify_customer_id', true );
	...
	public function update_failing_payment_method( $subscription, $renewal_order ) {
		...
		update_post_meta( $subscription->id, '_simplify_customer_id', get_post_meta( $renewal_order_id, '_simplify_customer_id', true ) );
	}
	...
	public function delete_resubscribe_meta( $resubscribe_order ) {
		$resubscribe_order_id = version_compare( WC_VERSION, '3.0', '<' ) ? $resubscribe_order->id : $resubscribe_order->get_id();
		delete_post_meta( $resubscribe_order_id, '_simplify_customer_id' );
	}
	...
	public function process_pre_order_release_payment( $order ) {
		...
		try {
			...
			// Charge the customer
			$payment = Simplify_Payment::createPayment( array(
				'amount'              => absint( $order->get_total() * 100 ), // In cents.
				'customer'            => $customer_id,
